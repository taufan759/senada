...
const Budget = () => {
  ...
  const deleteBudget = async (budgetId) => {
    try {
      const response = await fetch(`${AppSettings.api}/budget/delete/${budgetId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${AppSettings.api}/budget/add`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  const handleSubmitUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${AppSettings.api}/budget/update/${budgetId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
    ...
    <AppLayout>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8">
        ...
        <div className="flex gap-2">
          <button
            className='bg-secondary hover:bg-secondary-light text-white px-3 py-2 rounded-md font-medium transition-colors'
            onClick={() => setAddOpen(true)}
          >
            <span className='text-sm'>➕ Tambah Anggaran</span>
          </button>
        </div>
      </div>
      {/* Summary Cards */}
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 space-y-3 sm:space-y-0">
                    ...
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      ...
                      <button className="p-2 text-gray-400 hover:text-green-600 rounded-lg hover:bg-green-50 transition-colors"
                        onClick={() => setInputUpdate(budget.budgetId)}>
                        <span className="text-sm">✏️</span>
                      </button>
          ...
          {budgets.length === 0 && (
            <div className="text-center py-8 sm:py-12">
              ...
              <button
                ...
                onClick={() => setAddOpen(true)}
              >
                <span className='text-sm'>➕ Buat Anggaran</span>
              </button>
            </div>
          )}
        </div>
      ...
      <Modal open={addOpen} setOpen={setAddOpen} title="Tambah Anggaran">
      ...
      <Modal open={updateOpen} setOpen={setUpdateOpen} title="Update Anggaran">
        <form onSubmit={handleSubmitUpdate} className="space-y-6">
          <div className="grid grid-cols-1 gap-y-4 sm:grid-cols-6">
            <div className="sm:col-span-3">
              ...
              <SelectInput name="type" value={type} autoComplete="type-name" onChange={(e) => setType(e.target.value)} required>
                <option value="">Pilih Type</option>
                <option value="Income">💰Income</option>
                <option value="Expense">💸Expense</option>
              </SelectInput>
            ...
            <div className="sm:col-span-3">
              ...
              <SelectInput name="category" value={category} autoComplete="category-name" onChange={(e) => setCategory(e.target.value)} required>
                <option value="">Pilih Category</option>
                <option value="Gaji">💰Gaji</option>
                <option value="Bonus">🪙Bonus</option>
                <option value="Dividen">💹Dividen</option>
            ...
            <div className="sm:col-span-6">
              ...
              <InputText name="transaction" onChange={(e) => setBudgetName(e.target.value)} value={budgetName} placeHolder="Masukkan Nama Anggaran" required />
            </div>
            ...
            <div className="sm:col-span-3">
              ...
              <InputNumber name="allocated" onChange={(e) => setAllocated(e.target.value)} value={allocated} placeHolder="0.00" required />
            </div>
          </div>
